...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!expiryDate) {
      return NextResponse.json(
        { error: "Data trial tidak lengkap" },
        { status: 500 }
      );
    }
    ...
    const elapsedDays = Math.floor((now.getTime() - (license.activated_at ? new Date(license.activated_at).getTime() : now.getTime())) / (1000 * 60 * 60 * 24));
    const trialDays = 7;
    const daysLeft = Math.max(0, trialDays - Math.max(0, elapsedDays));

    // If trial was started purely via localStorage (no DB record with activated_at),
    // fall back to checking expires_at which was set by the admin trial route
    ...
    return NextResponse.json(
      { error: "Gagal memverifikasi trial" },
      { status: 500 }
    );
  }
}
