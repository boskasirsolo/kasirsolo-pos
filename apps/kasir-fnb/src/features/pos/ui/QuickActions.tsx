"use client";

interface QuickActionsProps {
  onHold: () => void;
  onVoid: () => void;
  onDiscount: () => void;
  onSendToKitchen: () => void;
  hasItems: boolean;
}

export function QuickActions({ onHold, onVoid, onDiscount, onSendToKitchen, hasItems }: QuickActionsProps) {
  const actions = [
    {
      label: "Tunda",
      icon: "M15.75 5.25v13.5m-7.5-13.5v13.5",
      onClick: onHold,
      disabled: !hasItems,
    },
    {
      label: "Diskon",
      icon: "M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z M6 6h.008v.008H6V6z",
      onClick: onDiscount,
      disabled: !hasItems,
    },
    {
      label: "Kirim Dapur",
      icon: "M15.362 5.214A8.252 8.252 0 0112 21 8.25 8.25 0 016.038 7.048 8.287 8.287 0 009 9.6a8.983 8.983 0 013.361-6.867 8.21 8.21 0 003 2.48z",
      onClick: onSendToKitchen,
      disabled: !hasItems,
      highlight: true,
    },
    {
      label: "Void",
      icon: "M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0",
      onClick: onVoid,
      disabled: !hasItems,
    },
  ];

  return (
    <div className="flex gap-1">
      {actions.map((action) => (
        <button
          key={action.label}
          onClick={action.onClick}
          disabled={action.disabled}
          className={`quick-action-btn ${action.disabled ? "opacity-40" : ""}
            ${action.highlight && !action.disabled ? "text-brand-primary" : ""}`}
          type="button"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d={action.icon} />
          </svg>
          <span>{action.label}</span>
        </button>
      ))}
    </div>
  );
}
