...
import { FNB_PAYMENT_METHODS, QUICK_AMOUNTS } from "../data/types";
import type { FnbPaymentMethod } from "../data/types";

interface PaymentModalProps {
  total: number;
  onClose: () => void;
  onComplete: (method: FnbPaymentMethod, amountPaid: number, paymentRef: string | null) => Promise<void>;
  onSplitBill: () => void;
  processing: boolean;
}

export function PaymentModal({ total, onClose, onComplete, onSplitBill, processing }: PaymentModalProps) {
  ...
  const isDelivery = method === "gofood" || method === "grabfood" || method === "shopeefood";
    ...
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative bg-white rounded-t-2xl max-h-[95vh] flex flex-col animate-slide-up">
        ...
        <div className="px-4 pb-3 border-b border-pos-border">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={onSplitBill}
                className="text-xs text-brand-primary font-medium px-3 py-1.5 rounded-lg
                  border border-brand-primary/30 hover:bg-brand-primary/5 transition-colors"
                type="button"
              >
                Split Bill
              </button>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
              {FNB_PAYMENT_METHODS.map((pm) => (
                ...
                >
                  ...
                  <span className="text-[9px] font-medium leading-tight text-center">{pm.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Non-cash / delivery reference */}
          {!isCash && (
            <div>
              <label className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-2 block">
                {isDelivery ? "ID Pesanan" : "Referensi (Opsional)"}
              </label>
              <input
                type="text"
                value={paymentRef}
                onChange={(e) => setPaymentRef(e.target.value)}
                placeholder={isDelivery ? "Masukkan ID pesanan platform" : "No. referensi / ID transaksi"}
