...
export function TopBar({ storeName, licenseStatus }: TopBarProps) {
  ...
  return (
    <header
      className="sticky top-0 z-30 bg-brand-dark text-white flex items-center justify-between px-4"
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-[10px] text-gray-400">{online ? 'Online' : 'Offline'}</span>
        </div>

        {/* Clock */}
        <span className="text-xs text-gray-300 font-mono tabular-nums">{time}</span>
      </div>
    </header>
  );
}
